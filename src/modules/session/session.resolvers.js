module.exports = {
    Query: {
        getSessionData: async(parent, { id }, { mongo } ) => {
            return await mongo.Session.findById(id);
        }
    },
    Session: {
    ...
    Mutation: {
        ...
        closeSession: async (parent, { sessionId }, { mongo, pubsub }) => {
            try {
              const updatedSession  = await mongo.Session.findByIdAndUpdate(sessionId, {open: false}, { new: true});
    ...
    Subscription: {
        liveSession: {
            subscribe: (parent, { sessionId }, { pubsub }) => {
              return pubsub.asyncIterator(sessionId);
            },
        }
    }
}